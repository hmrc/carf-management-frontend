...
class OrganisationSecondContactHavePhoneController @Inject() (
              ...
              case ChangeMode =>
                Redirect {
                  if (hasValueChanged(value)) {
                    navigateFromOrganisationSecondContactHavePhonePage(value)
                  } else {
  ...
  private def navigateFromOrganisationSecondContactHavePhonePage(havePhone: Boolean): Call =
    if (havePhone) controllers.organisation.routes.OrganisationSecondContactPhoneNumberController.onPageLoad(ChangeMode)
