...
class OrganisationFirstContactHavePhoneController @Inject() (
  ...
  def onSubmit(mode: Mode): Action[AnyContent] =
    (identify() andThen getData() andThen submissionLock andThen requireData).async { implicit request =>
      ...
      lazy val hasValueChanged: Boolean => Boolean =
        newValue => !userAnswers.get(OrganisationFirstContactHavePhonePage).contains(newValue)
      form
        .bindFromRequest()
        .fold(
          formWithErrors =>
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(OrganisationFirstContactHavePhonePage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield mode match {
              case NormalMode =>
                Redirect(navigator.nextPage(OrganisationFirstContactHavePhonePage, mode, updatedAnswers))
              case ChangeMode =>
                Redirect {
