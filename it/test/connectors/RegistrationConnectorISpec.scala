...
import models.responses.{AddressRegistrationResponse, RegisterWithIdResponse}
...
class RegistrationConnectorISpec
  ...
  val validRegisterWithIdRequest: RegisterWithIdRequest = RegisterWithIdRequest(
    requiresNameMatch = false,
    IDNumber = "testIDNumber",
    IDType = "UTR"
  )

  val validRegisterWithIdResponse: RegisterWithIdResponse = RegisterWithIdResponse(
  ...
  "registerOrganisationWithUtrCtAutoMatch" should {
    "successfully retrieve a name and address" in {
      ...
      val result = connector.registerOrganisationWithUtrCtAutoMatch(validRegisterWithIdRequest).value.futureValue

      result shouldBe Right(validRegisterWithIdResponse)
    }
